				...
				for(var i=0;i<tds.length;i++){
					...
					for(var j=0;j<table.length;j++){
						...
						if(dateformat(thisday)==jsonDateFormat(table[j].date)){
							...
							"<br/>上班时间：<input type='time' name='startTime' style='width:65px; height:18px; margin-top:1px' value='"+jsonTimeFormat(table[j].startTime)+"' />"+
							"<br/>下班时间：<input type='time' name='endTime' style='width:65px; height:18px; margin-top:1px' value='"+jsonTimeFormat(table[j].endTime)+"' />"+
							"<input type='hidden' name='scheduleTableId' value='"+table[j].scheduleTableId+"' />";
							...
							if(table[j].ifOnduty==2){
								...
								"<input type='radio' name='flag"+j+"' value='false' style='margin-top:2px' checked='checked' />否"+
										"<br/><label style='display:none'>值班类型：</label>";
							}else{
								...
								if(table[j].typeOnduty==1){
									tag+="<br/><label>值班类型：</label><select name='typeOnduty' ><option value='1'>工作日值班</option>"+
									"<option value='2'>周末值班</option></select>";
								}else{
									tag+="<br/><label>值班类型：</label><select name='typeOnduty' ><option value='2'>周末值班</option>"+
